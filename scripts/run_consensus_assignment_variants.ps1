param(
    [string[]]$Modes = @("consensus_lock", "union_restrict", "bilateral_gain")
)
...
$output = Join-Path $project "results\consensus_assignment_functional_recovery_2of64"
New-Item -ItemType Directory -Force -Path $output | Out-Null

$processes = @()
foreach ($mode in $Modes) {
    if ($mode -notin @("consensus_lock", "union_restrict", "bilateral_gain")) {
        throw "Unsupported consensus assignment mode: $mode"
    }
    ...
    $arguments = @(
        ...
        "--functional-assignment-mode", $mode,
        "--run-name", $name
    )
...
for ($index = 0; $index -lt $processes.Count; $index++) {
    [pscustomobject]@{ pid = $processes[$index].Id; mode = $Modes[$index] }
}
