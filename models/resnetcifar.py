...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class ResNetTinyImageNet_Drop(nn.Module):
    def __init__(self, block, layers, num_classes=200, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, dropout_rate=0.5, feature_dropout_rate=0.5): 
        super(ResNetTinyImageNet_Drop, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        # First conv layer remains same as it works well for 64x64 input
...
class ResNetTinyImageNet_Drop25(nn.Module):
    def __init__(self, block, layers, num_classes=200, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, dropout_rate=0.25, feature_dropout_rate=0.25): 
        super(ResNetTinyImageNet_Drop25, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
class ResNetCifar10_drop(nn.Module):
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        # Add dropout rates as parameters
        self.dropout_rate = dropout_rate
        self.feature_dropout_rate = feature_dropout_rate
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer1(x)
        x = self.feature_dropout(x)  # Add dropout after layer1
        result['activation1'] = x
        
        x = self.layer2(x)
        x = self.feature_dropout(x)  # Add dropout after layer2
        result['activation2'] = x
        
        x = self.layer3(x)
        x = self.feature_dropout(x)  # Add dropout after layer3
        result['activation3'] = x
        
        x = self.layer4(x)
        x = self.feature_dropout(x)  # Add dropout after layer4
        result['activation4'] = x
        ...
        x = self.dropout(x)  # Add dropout before final FC layer
        x = self.fc(x)
        result['output'] = x
...
def ResNet18_cifar10_drop(**kwargs):
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return ResNetCifar10_drop(BasicBlock, [2, 2, 2, 2], **kwargs)
